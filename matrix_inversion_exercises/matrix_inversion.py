...
def gauss_jordan_lower_triangular(matrix: Matrix, inv_matrix: Matrix) -> tuple[Matrix]:
    """
    ...
    for actual_index in range(len(matrix) - 1):
        ...
        if matrix.index(pivot_row) != actual_index:
            inv_matrix = invert_row(inv_matrix, matrix.index(pivot_row), actual_index)
            matrix = invert_row(matrix, matrix.index(pivot_row), actual_index)
...
def gauss_jordan_upper_triangular(matrix: Matrix, inv_matrix: Matrix) -> Matrix:
    """
...
def gauss_jordan_inversion_matrix(matrix: Matrix) -> Matrix | None:
    ...
    matrix, inv_matrix = gauss_jordan_upper_triangular(
        *gauss_jordan_lower_triangular(matrix, inv_matrix)
    )
